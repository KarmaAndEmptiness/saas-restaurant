...
CREATE TABLE `saas_restaurant`.`inventory`  (
  ...
  `min_stock` int NULL COMMENT '库存预警阈值',
...
CREATE TABLE `saas_restaurant`.`marketing_campaign`  (
  ...
  `campaign_content` json NULL COMMENT '活动内容',
...
CREATE TABLE `saas_restaurant`.`order`  (
  ...
  `payment_method_id` varchar(255) NULL COMMENT '支付方式ID',
